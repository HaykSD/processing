class System {
  ...
  ArrayList<Ball> make_particle_list() {
    ...
    for (int i=0; i<balls_count; i++) {
      ...
      b_vel = new PVector(random(-v_range, v_range)/10, random(-v_range, v_range)/10);
      //Ball b = new Ball(b_pos, b_vel, floor(random(10, 20)), color(random(0, 100), random(100, 255), random(100, 240), 255));
       Ball b = new Ball(b_pos, b_vel, floor(random(11)), color(192,192,192));
      //b.applyՀամազորForce(new PVector(0,random(-1,1)));
  ...
  void draw_lines(ArrayList<Ball> balls, float radius_of_visibility) {
    ...
    for (int i=0; i<balls.size(); i++) {
      ...
      for (int j=0; j<i; j++) {
        ...
        color c = img.get(int(i_ball.pos.x),int(i_ball.pos.y));

        if (d<max_dist  && (brightness(c)>88)) {
          
          stroke(c);
          strokeWeight(map(d, 0, max_dist, 7, 0));
        } else {
          noStroke();
        }
